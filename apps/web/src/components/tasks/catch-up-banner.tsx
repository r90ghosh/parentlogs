    ...
    >
      ...
      <div className="flex flex-col md:flex-row md:items-center gap-4">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3 md:hidden">
            <div className="text-center">
              <div className="text-2xl font-ui font-extrabold text-[--cream]">
                {catchUpCount}
              </div>
              <div className="text-[10px] text-gold font-body">catch up</div>
            </div>
            {autoHandledCount > 0 && (
              <div className="text-center">
                <div className="text-lg font-ui font-bold text-[--muted]">
                  {autoHandledCount}
                </div>
                <div className="text-[10px] text-[--muted] font-body">auto-sorted</div>
              </div>
            )}
            <div className="w-16">
              <div className="h-1.5 bg-[--card] rounded-full mb-1 overflow-hidden">
                <div
                  className="h-full bg-gradient-to-r from-gold to-copper rounded-full transition-all duration-300"
                  style={{ width: `${percentDone}%` }}
                />
              </div>
              <div className="text-[10px] text-[--muted] font-body text-center">
                {percentDone}% done
              </div>
            </div>
          </div>
        ...
        <div className="flex-1">
          <h3 className="text-base md:text-lg font-display font-bold text-[--cream] mb-1">
          ...
          <p className="text-xs md:text-sm text-[--muted] font-body leading-relaxed">
            ...
            {autoHandledCount > 0 && (
              <span className="text-[--dim]"> {autoHandledCount} were auto-handled, {catchUpCount} need your review.</span>
            )}
          </p>
        </div>
        ...
        <div className="hidden md:flex items-center gap-4">
          <div className="text-center">
            <div className="text-[32px] font-ui font-extrabold text-gold">
              {catchUpCount}
            </div>
            <div className="text-xs text-gold/70 font-body">catch up</div>
          </div>

          {autoHandledCount > 0 && (
            <div className="text-center">
              <div className="text-xl font-ui font-bold text-[--muted]">
                {autoHandledCount}
              </div>
              <div className="text-xs text-[--muted] font-body">auto-sorted</div>
            </div>
          )}

          <div className="w-[120px]">
            <div className="h-2 bg-[--card] rounded-full mb-1.5 overflow-hidden">
              <div
                className="h-full bg-gradient-to-r from-gold to-copper rounded-full transition-all duration-300"
                style={{ width: `${percentDone}%` }}
              />
            </div>
            <div className="text-[11px] text-[--muted] font-body text-center">
              {percentDone}% done
            </div>
          </div>
        </div>
        ...
        <Link
          ...
          className={cn(
            'px-4 py-2.5 md:px-6 md:py-3 rounded-[10px] text-sm font-ui font-semibold text-[--bg] text-center',
            'bg-gradient-to-r from-gold to-copper',
